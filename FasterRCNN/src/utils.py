...
def collate_fn_YOLO(batch):
    ...
    of objects and to handle varying size tensors as well.
    """
    images, bb_targets = list(zip(*batch))
...
def rotate_90(images, targets):
    # images = torch.rot90(images)
    # targets not rotated yet
    return images, targets
...
# Type - GT (ground truth) or DETECTED (detected)
def vocpascal2bb(annotations_path, _type='GT'):
    ret = []
    # Get annotation files in the path
    annotation_files = glob.glob(f"{annotations_path}/*.xml")
    ...
    for file_path in annotation_files:
        ...
        for box_info in ET.parse(file_path).iter('object'):
            ...
            if _type == 'DETECTED':
                confidence = float(box_info.find('score').text)
