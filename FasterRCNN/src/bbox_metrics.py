...
def get_pascalvoc_metrics(gt_boxes,
    ...
    for c, v in classes_bbs.items():
        ...
        # print(TP)
        # if first_time == False:
        #     TP_all = np.append(TP_all, TP)
        #     FP_all = np.append(FP_all, TP)
        #     npos_all += npos
        # else:
        #     TP_all = TP
        #     FP_all = TP
        #     npos_all = npos
        #     first_time = False
    # compute precision, recall and average precision for all classes
    # acc_FP_all = np.cumsum(FP_all)
    # acc_TP_all = np.cumsum(TP_all)
    # prec_all = np.divide(acc_TP_all, (acc_FP_all + acc_TP_all))
    # print(prec_all)
    # rec_all = acc_TP_all / npos_all
...
def plot_precision_recall_curves_all(results,
                                 showAP=False,
                                 showInterpolatedPrecision=False,
                                 savePath=None,
                                 showGraphic=True):
    ...
    for classId, result in results.items():
        ...
        method = result['method']
        # plt.close()
        ap_str = "{0:.3f}".format(average_precision)
        plt.plot(recall, precision, label=str(classId)+' '+ap_str, linewidth=2)

    # ap_str = "{0:.3f}".format(result['AP'])
    # plt.plot(results_all['recall'], results_all['precision'], label='all classes '+ap_str+' mAP@0.5')    
    plt.xlabel('recall')
    plt.ylabel('precision')
    plt.xlim([0, 1.005])
    plt.ylim([0, 1.005])
